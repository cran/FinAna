...
desc <- function(x){
  ...
  des[,3] <- apply(x,2, max)
  des[,4] <- apply(x,2, min)
  des[,5] <- apply(x,2,mean, na.rm = T)
  des[,6] <- apply(x,2,median, na.rm = T)
  des[,7] <- apply(x,2,get.mode)
  des[,8] <- apply(x,2,var, na.rm = T)
  des[,9] <- apply(x,2,sd, na.rm = T)
  des[,10] <- apply(x,2,sk)
  des[,11] <- apply(x,2,kur)
  return(des)
}
